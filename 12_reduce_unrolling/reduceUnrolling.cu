...
//
//
__global__ void reduceCompleteUnrollWarp8(int *g_idata, int *g_odata,
                                          unsigned int n) {
  // set thread ID
  unsigned int tid = threadIdx.x;
  unsigned int idx = blockDim.x * blockIdx.x * 8 + threadIdx.x;
...
int main(int argc, char **argv) {
  ...
  cudaDeviceSynchronize();
  iElaps = cpuSecond() - iStart;
  cudaMemcpy(odata_host, odata_dev, grid.x * sizeof(int),
             cudaMemcpyDeviceToHost);
  gpu_sum = 0;
  for (int i = 0; i < grid.x / 2; i++)
    gpu_sum += odata_host[i];
  printf("reduceUnrolling2            elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
  ...
  for (int i = 0; i < grid.x / 4; i++)
    gpu_sum += odata_host[i];
  printf("reduceUnrolling4            elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
  ...
  for (int i = 0; i < grid.x / 8; i++)
    gpu_sum += odata_host[i];
  printf("reduceUnrolling8            elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
  ...
  for (int i = 0; i < grid.x / 8; i++)
    gpu_sum += odata_host[i];
  printf("reduceUnrollingWarp8        elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
         iElaps, gpu_sum, grid.x / 8, block.x);
  ...
  for (int i = 0; i < grid.x / 8; i++)
    gpu_sum += odata_host[i];
  printf("reduceCompleteUnrollWarp8   elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
         iElaps, gpu_sum, grid.x / 8, block.x);
  ...
  case 1024:
    reduceCompleteUnroll<1024>
        <<<grid.x / 8, block>>>(idata_dev, odata_dev, size);
    break;
  case 512:
  ...
  for (int i = 0; i < grid.x / 8; i++)
    gpu_sum += odata_host[i];
  printf("reduceCompleteUnroll        elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
         iElaps, gpu_sum, grid.x / 8, block.x);
  // free host memory
