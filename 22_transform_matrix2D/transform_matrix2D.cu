...
__global__ void transformNaiveRowUnroll(float *MatA, float *MatB, int nx,
                                        int ny) {
...
__global__ void transformNaiveColUnroll(float *MatA, float *MatB, int nx,
                                        int ny) {
  int ix = threadIdx.x + blockDim.x * blockIdx.x * 4;
  int iy = threadIdx.y + blockDim.y * blockIdx.y;
  int idx_row = ix + iy * nx;
...
__global__ void transformNaiveRowDiagonal(float *MatA, float *MatB, int nx,
                                          int ny) {
...
__global__ void transformNaiveColDiagonal(float *MatA, float *MatB, int nx,
                                          int ny) {
  int block_y = blockIdx.x;
  int block_x = (blockIdx.x + blockIdx.y) % gridDim.x;
  int ix = threadIdx.x + blockDim.x * block_x;
