...
int main(int argc, char **argv) {
  ...
  printf("gpu warmup                 elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
  ...
  for (int i = 0; i < grid.x; i++)
    gpu_sum += odata_host[i];
  printf("gpu reduceNeighbored       elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
         iElaps, gpu_sum, grid.x, block.x);
  ...
  for (int i = 0; i < grid.x; i++)
    gpu_sum += odata_host[i];
  printf("gpu reduceNeighboredLess   elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
         iElaps, gpu_sum, grid.x, block.x);
  ...
  for (int i = 0; i < grid.x; i++)
    gpu_sum += odata_host[i];
  printf("gpu reduceInterleaved      elapsed %lf ms gpu_sum: %d<<<grid %d "
         "block %d>>>\n",
         iElaps, gpu_sum, grid.x, block.x);
  // free host memory
