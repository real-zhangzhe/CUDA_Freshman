...
int main(int argc, char **argv) {
  ...
  case 6: {
    setRowReadColDynIpad<<<grid, block, (BDIMX + IPAD) * BDIMY * sizeof(int)>>>(
        out);
    cudaDeviceSynchronize();
    iElaps = cpuSecond() - iStart;
  ...
  case 8: {
    setRowReadColRectDyn<<<grid_rect, block_rect,
                           (BDIMX)*BDIMY * sizeof(int)>>>(out);
    cudaDeviceSynchronize();
    iElaps = cpuSecond() - iStart;
  ...
  case 10: {
    setRowReadColRectDynPad<<<grid_rect, block_rect,
                              (BDIMX + 1) * BDIMY * sizeof(int)>>>(out);
    cudaDeviceSynchronize();
    iElaps = cpuSecond() - iStart;
